import {BetweenValueValidationRule} from '../../src/validation/validation-rules';
...
describe('Tests on BetweenValueValidationRule', () => {
  ...
  it('should be working with decimals', (done) => {
    ...
    expectations.expectAsync(rule.validate(3.99)).toBe(true);
  ...
  it('should be working with dates', (done) => {
    ...
    expectations.expectAsync(rule.validate(new Date(2000, 1, 1, 1, 1, 2, 1))).toBe(false);
    expectations.validate();
  });
});
