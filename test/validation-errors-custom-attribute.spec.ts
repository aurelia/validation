...
describe('ValidationErrorsCustomAttribute', () => {
  ...
  const stageTest = (validationErrors: string, supplyControllerToViewModel?: boolean) => {
    ...
    // needed for webpack to recognize the module in one of its bundles
    PLATFORM.moduleName('test/resources/validation-errors-form-one', 'form-one');

    component = StageComponent
      .withResources()
      // tslint:disable-next-line:max-line-length
      .inView(`<compose containerless view-model="test/resources/validation-errors-form-one" model.bind="{ form: form, controller: controller }"></compose>`)
